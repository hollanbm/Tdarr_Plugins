...
const details = () => ({
  id: 'Tdarr_Plugin_hollanbm_CleanSubs',
  Stage: 'Pre-processing',
  Name: 'Brad-Clean subtitle streams',
  Type: 'Subtitle',
  Operation: 'Transcode',
  ...
  },
  {
    ...
    tooltip: `Specify if subtitle tracks that contain signs_songs/description should be removed.
               \\nExample:\\n
               true
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  for (let i = 0; i < file.ffProbeData.streams.length; i++) {
    ...
    try {
      // Check if inputs.signs_songs is set to true
      // AND if stream is subtitle
      // AND then checks for stream titles with the following "signs_songs, description, sdh".
      // Removing any streams that are applicable.
      if (inputs.signs_songs === true
        && engSubtitleCount > 1
        && file.ffProbeData.streams[i].codec_type.toLowerCase() === 'subtitle'
